...
(function (w) {
  const KEY = "nh_voice";
  const TYPES = [
    { id: "yujie", name: "御姐", pitch: 0.96, rate: 0.96, prefer: ["xiaoxiao", "xiaoxuan", "huihui"] },
    { id: "loli", name: "萝莉", pitch: 1.12, rate: 1.06, prefer: ["xiaoyi", "yaoyao"] },
    { id: "dia", name: "嗲嗲", pitch: 1.18, rate: 0.98, prefer: ["xiaobei", "xiaoxiao", "yaoyao"] },
    { id: "dahai", name: "大海", pitch: 0.88, rate: 0.78, prefer: ["xiaoxiao", "xiaohan", "huihui"],
      preview: "嗯……叫大海。今晚把你弄到腿软，出牌我会浪给你听" },
    { id: "jieliu", name: "街溜子", pitch: 1.0, rate: 1.04, prefer: ["yunjian", "yunxi", "kangkang"],
      preview: "街溜子来了，报牌全他妈脏话，接不住就滚" },
    { id: "dashu", name: "大叔", pitch: 0.9, rate: 0.88, prefer: ["yunyang", "yunjian", "kangkang"] },
    { id: "youth", name: "青年", pitch: 1.02, rate: 1.0, prefer: ["yunxi", "yunjian", "kangkang"] }
  ];
  ...
  function pickVoice(type) {
    ...
    const neural = pool.filter(v => /natural|online|neural|yun|xiao/i.test(v.name));
  ...
  function flavor(typeId, text) {
    ...
    if (typeId === "jieliu") {
      const raw = text.replace(/^杀，?/, "");
      if (/不出/.test(text)) return pick(["不出你妈的", "过你妈逼，怂了", "不出？阳痿啊", "过，下把弄死你"]);
  ...
  function speak(typeId, text, opt) {
    ...
    else if (kind === "pass") file = typeId === "jieliu" ? pick(["pass.mp3", "pass2.mp3", "pass3.mp3"]) : "pass.mp3";
  ...
  w.NightVoice = {
    TYPES, getType, speak, speakCards, cardText, cardsText, clipKey,
